...
setInterval(() => {
    let queuedJobs = getAllQueuedJobs();
    if (queuedJobs.length === 0) {
        return;
    }
    queuedJobs.forEach((job, index) => {
        io.emit(`job-${job.getJobId()}`, { status: job.getStatus(), queuePosition: index + 1 });
    });
    const channel = getFreeChannel();
    if (!channel) {
        return;
    }
    giveJobToChannel(channel, queuedJobs[0]);
}, 1400);
...
const saveGeneratedImage = (message, job) => {
    ...
    axios.downloadImage(imageUrl).then(res => {
        s3.uploadImage(message.attachments.first().name, res.data).then(() => {
            s3.getImageUrl(message.attachments.first().name).then(imageUrl => {
                //TODO: this goes to updateJobStatus
                job.status = Status.READY;
                io.emit(`job-${job.getJobId()}`, { status: job.getStatus(), imageUrl });
            });
